...
def _column(frame: pd.DataFrame, name: str) -> pd.Series:
    return (
        frame[name] if name in frame.columns else pd.Series([None] * len(frame), index=frame.index)
    )
...
def _normalize(frame: pd.DataFrame, indicator: str) -> pd.DataFrame:
    ...
    output[f"mainNetInflow{'' if indicator == '今日' else indicator}"] = _column(
        frame, f"{prefix}主力净流入-净额"
    ).map(_number)
    if indicator == "今日":
...
def fetch_payload(provider: AkShareProvider, data_date: str | None = None) -> dict[str, object]:
    frames = [
        _normalize(provider.get_individual_fund_flow_rank(window), window)
        for window in ("今日", "3日", "5日", "10日")
    ]
    merged = frames[0]
        ...
        rows.append(
            {
                ...
                "mainNetInflowPct": row.get("mainNetInflowPct"),
                "superLargeNetInflow": row.get("superLargeNetInflow"),
                "largeNetInflow": row.get("largeNetInflow"),
                "mediumNetInflow": row.get("mediumNetInflow"),
                "smallNetInflow": row.get("smallNetInflow"),
                "mainNetInflow3d": row.get("mainNetInflow3日"),
                "mainNetInflow5d": row.get("mainNetInflow5日"),
                "mainNetInflow10d": row.get("mainNetInflow10日"),
            }
        )
    return {
...
def main() -> int:
    ...
    payload["rows"] = filter_rows_to_universe(payload["rows"], load_universe_codes(args.universe))
    if not payload["rows"]:
        raise RuntimeError("money-flow snapshot contains no rows from the current stock universe")
